  ...
  module Collection
    ...
    module ClassMethods
      ...
      def tag_scope_joins(tag_scope, tagging_scope)
        tag_scope = tag_scope.joins("JOIN (#{safe_to_sql(tagging_scope)}) #{ActsAsTaggableOn::Tag.using_oracle_enhanced? ? '' : 'AS '}#{ActsAsTaggableOn::Tagging.table_name} ON #{ActsAsTaggableOn::Tagging.table_name}.tag_id = #{ActsAsTaggableOn::Tag.table_name}.id")
